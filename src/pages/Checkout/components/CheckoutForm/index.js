...
export const CheckoutFormRaw = ({
  ...
  const CheckoutSchema = Yup.object().shape({
    phone: Yup.string()
      .required(t("validation.required", { field: t("checkout.form.phone") }))
      .test(
  ...
  const getShippingType = () => {
    return ShippingStore.items.find(
      (item) => item.id === +formik.values.shipping_id
    );
  };

  const getPaymentType = () => {
    return PaymentStore.items.find(
      (item) => item.id === +formik.values.payment_id
    );
  };
...
export const CheckoutForm = inject(
  "PaymentStore",
  "ShippingStore",
  "CartStore",
  "SpotsStore"
)(observer(CheckoutFormRaw));
